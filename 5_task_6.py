...
with open('Pretask_6.txt', 'r', encoding='utf-8') as f:
    for line in f:
        subject, lesson = line.split(':')
        el = [i for i in lesson if i == ' ' or (i >= '0' and i <= '9')]
        subject_qty = sum(map(int, ''.join(el).split()))
        dict[subject] = subject_qty
